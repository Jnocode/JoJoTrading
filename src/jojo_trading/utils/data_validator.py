...
class FinancialDataValidator:
    ...
    def validate_data_quality(self, financial_data) -> float:
        """
        簡化的數據品質評估方法（與測試腳本兼容）
        
        Args:
            financial_data: 財務數據（可以是字典或 DataFrame）
        ...
        try:
            ...
            if hasattr(financial_data, 'to_dict'):
                ...
                if len(financial_data) > 0:
                    ...
                    for column in financial_data.columns:
                        if column in ['revenue', 'operating_income', 'net_income', 'cash']:
                            values = financial_data[column].dropna()
                            if len(values) > 1:
                                # 計算變異係數（較低的變異係數代表更穩定）
                                cv = abs(values.std()) / abs(values.mean()) if values.mean() != 0 else 1.0
                                # 轉換為品質分數（變異係數越低，品質越高）
                                stability_score = max(0.0, 1.0 - min(cv, 1.0))
                                
                                # 檢查成長趨勢一致性
                                growth_rates = values.pct_change().dropna()
                                growth_consistency = 1.0 - (abs(growth_rates.std()) / 0.5) if len(growth_rates) > 0 else 0.5
                                growth_consistency = max(0.0, min(1.0, growth_consistency))
                                
                                # 檢查是否有負值（對於收入類指標）
                                negative_penalty = 0.3 if (values < 0).any() and column in ['revenue', 'operating_income'] else 0.0
                                
                                # 綜合品質分數
                                field_quality = (stability_score * 0.5 + growth_consistency * 0.5) - negative_penalty
                                quality_scores.append(max(0.0, field_quality))
                ...
                # 使用現有的驗證方法
                validation_result = self.validate_financial_data("TEST", data_dict)
            ...
            print(f"    (DataValidator) 品質評估錯誤: {e}")
            return 0.5  # 預設中等品質
