...
def get_config_summary() -> str:
    """取得所有配置的摘要說明"""    summary = "JoJoTrading 成長股篩選優化配置\n"
    summary += "=" * 40 + "\n\n"
    ...
    for name, config in OPTIMIZED_CONFIGS.items():
        ...
        for criterion in config.criteria_set.criteria:
            ...
            label_text = criterion.label if criterion.label else "未知條件"
            formatted_label = label_text.replace(f'> {threshold_pct:.0f}%', f'> {threshold_pct:.1f}%') if '>' in label_text else label_text
            summary += f"     • {formatted_label}\n"
        summary += "\n"
