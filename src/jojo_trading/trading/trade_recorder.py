...
class TradeEntry:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'TradeEntry':
        ...
        entry.entry_time = datetime.fromisoformat(data.get('entry_time', datetime.now().isoformat()))
        entry.quantity = data.get('quantity', 0)
        entry.exit_price = data.get('exit_price')
        entry.exit_time = datetime.fromisoformat(data['exit_time']) if data.get('exit_time') else None
...
class TradeRecorder:
    ...
    def add_trade(self, trade: TradeEntry) -> str:
        ...
        self.save_trades()
        print(f"📝 新增交易記錄: {trade.stock_code} {trade.trade_type.value} {trade.quantity}股 @ ${trade.entry_price}")
        return trade.trade_id
    
    def close_trade(self, trade_id: str, exit_price: float, exit_quantity: Optional[int] = None) -> bool:
        """平倉交易"""
        for trade in self.trades:
            if trade.trade_id == trade_id and trade.status == TradeStatus.OPEN:
                ...
                if trade.trade_type == TradeType.BUY:
                    trade.realized_pnl = (exit_price - trade.entry_price) * trade.exit_quantity
                else:
                    trade.realized_pnl = (trade.entry_price - exit_price) * trade.exit_quantity
                
                trade.return_percentage = (trade.realized_pnl / (trade.entry_price * trade.exit_quantity)) * 100
                ...
                self.save_trades()
                print(f"💰 平倉交易: {trade.stock_code} 盈虧: ${trade.realized_pnl:.2f} ({trade.return_percentage:.2f}%)")
                return True
        return False
    ...
    def calculate_portfolio_performance(self) -> Dict[str, Any]:
        ...
        total_pnl = sum(trade.realized_pnl for trade in closed_trades if trade.realized_pnl)
        winning_trades = [trade for trade in closed_trades if trade.realized_pnl and trade.realized_pnl > 0]
        returns = [trade.return_percentage for trade in closed_trades if trade.return_percentage]
