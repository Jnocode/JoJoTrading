...
class SignalGenerator:
    ...
    def __init__(self, trade_recorder: TradeRecorder, ai_advisor: AITradingAdvisor):
        ...
        for stock_data in stock_list:
            try:
                ...
                if signal and signal.confidence >= self.confidence_threshold:
                    ...
                    print(f"📡 發現信號: {signal.stock_code} {signal.action.value} "
                         f"信心度{signal.confidence:.0f}% 目標${signal.target_price:.2f}")
            except Exception as e:
                print(f"⚠️ 分析股票 {stock_data.get('stock_code', 'Unknown')} 時發生錯誤: {e}")
    ...
    def _evaluate_single_stock(self, stock_data: Dict[str, Any]) -> Optional[TradingSignal]:
        """評估單一股票並生成信號"""
    ...
    def _refine_signal(self, signal: TradingSignal, analysis: StockAnalysis, 
                      stock_data: Dict[str, Any]) -> TradingSignal:
        """優化和細化信號"""
        ...
        # 基於交易歷史調整
        historical_performance = self.ai_advisor._get_stock_historical_performance(signal.stock_code)
        ...
        # 基於持倉情況調整
        open_positions = len([t for t in self.trade_recorder.get_open_trades() 
    ...
    def _assess_market_condition(self) -> str:
        """評估整體市場狀況（簡化版）"""
        # 基於最近交易活動評估市場
        recent_trades = [t for t in self.trade_recorder.trades 
    ...
    def _should_exit_position(self, trade: TradeEntry, current_price: float) -> Optional[str]:
        """判斷是否應該出場"""
...
class AutoTradingEngine:
    ...
    def __init__(self, trade_recorder: TradeRecorder, signal_generator: SignalGenerator):
        self.trade_recorder = trade_recorder
    ...
    def _execute_entry_trade(self, entry_signal: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if quantity > 0:
                ...
                print(f"✅ 自動執行進場: {signal.stock_code} {signal.action.value} {quantity}股")
                return True
    ...
    def _execute_exit_trade(self, exit_signal: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if success:
                print(f"✅ 自動執行出場: {exit_signal['stock_code']} 原因: {exit_signal['exit_reason']}")
...
# 便利函數
def create_signal_generator(trade_recorder: TradeRecorder, ai_advisor: AITradingAdvisor) -> SignalGenerator:
    """創建信號生成器"""
    return SignalGenerator(trade_recorder, ai_advisor)

def create_auto_trading_engine(trade_recorder: TradeRecorder, signal_generator: SignalGenerator) -> AutoTradingEngine:
    """創建自動交易引擎"""
