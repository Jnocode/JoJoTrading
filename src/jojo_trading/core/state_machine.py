...
from src.jojo_trading.analysis.growth_analyzer import (
...
class ConfigLoadState(State):
    def execute(self):
        ...
        try:
            ...
            print(
                f"  industries.json 已載入。偵測到產業數量: {len(self.context['industry_data'].get('industries', []))}"
            )

            # 從 industry_data 提取產業名稱列表和風險選項
            industries_list = self.context["industry_data"].get(
                "industries", []
            )  # 'industries' is a list
            self.context["industry_names"] = [
                item["name"]
            ...
            risk_premiums_config = self.context["industry_data"].get(
                "risk_premium_options", {}
            )
            self.context["risk_premium_options"] = risk_premiums_config

            # default_risk_premium is a top-level key in industries.json
            self.context["default_risk_premium"] = self.context["industry_data"].get(
                "default_risk_premium", 0.04
            )
            ...
            print(
                f"  DCF 方法設定: {'增強版' if data_handler.USE_ENHANCED_DCF else '原始版'}"
            )
            ...
            if not self.context.get("all_companies_openapi_data"):
                self.context["all_companies_openapi_data"] = (
                    data_handler.get_all_companies_basic_data(self.context)
                )
...
class DataFetchState(State):
    def execute(self):
        ...
        selected_industry_name = self.context.get(
            "selected_industry_name"
        )  # Use the correct context key
        selected_stock_codes = self.context.get("selected_stock_codes", [])
        ...
        if not self.context.get("all_companies_openapi_data"):
            ...
            if not self.context.get(
                "all_companies_openapi_data"
            ):  # Still none after trying
        ...
        if selected_stock_codes and not selected_industry_name:
            ...
            self.context["industry_stocks_details"] = [
                {"code": str(c.get("公司代號")), "name": c.get("公司名稱"), **c}
                for c in all_companies
                if isinstance(c, dict)
                and str(c.get("公司代號"))
                in [str(code) for code in selected_stock_codes]
            ]
        else:
            # 產業模式（維持原本 code 欄位邏輯）
            ...
            if selected_stock_codes:  # 產業模式下，若有選個股，再篩一次
                self.context["industry_stocks_details"] = [
                    s
                    for s in self.context["industry_stocks_details"]
                    if s["code"] in selected_stock_codes
                ]

        if not self.context["industry_stocks_details"]:
            print(
                f"  (DataFetchState) 查無任何待處理股票（產業: '{selected_industry_name}', 個股: {selected_stock_codes}）。"
            )
            self.context["processed_data"] = {}  # Ensure it's empty
            self.machine.transition_to(
                JoJoState.VALUATION
            )  # Proceed to valuation, which will handle empty data
            return
        ...
        if not self.context.get("all_stock_prices_openapi_data"):
            try:
                ...
                response = api_request_with_retry(
                    f"{data_handler.API_BASE_URL}/t187ap03_L", timeout=30, verify=False
                )
                self.context["all_stock_prices_openapi_data"] = response.json()
                print(
                    f"    (DataFetchState) 成功獲取 {len(self.context['all_stock_prices_openapi_data'])} 筆股價記錄。"
                )
            except Exception as e:
        ...
        for stock_detail in self.context["industry_stocks_details"][
            :3
        ...
        print(
            f"  (DataFetchState) 預計處理 {total_stocks} 支股票 (上限: {'無限制' if max_stocks == 0 else max_stocks})..."
        )
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            # 提交所有任務
            future_to_stock = {
                executor.submit(fetch_single_stock, stock): stock
                ...
                try:
                    ...
                    if dcf_data["success"]:
                        ...
                        if "eps_finmind" not in financial_data_for_stock:
                            try:
                                ni = financial_data_for_stock.get(
                                    "net_income_parent", 0
                                )
                                shares = financial_data_for_stock.get(
                                    "shares_outstanding", 1
                                )
                                if shares > 0:
                                    financial_data_for_stock["eps_finmind"] = (
                                        ni / shares
                                    )
                            except:
                                pass

                        all_financial_data_for_stock[stock_code] = (
                            financial_data_for_stock
                        )
                        ...
                        print(
                            f"    (DataFetchState) 獲取 {stock_code} 數據失敗: {error_msg}"
                        )
...
class ValuationState(State):
    def execute(self):
        ...
        valuation_results = []
        risk_preference = self.context.get(
        ...
        for stock_code, financials in self.context.get("processed_data", {}).items():
            ...
            if financials.get("error") and "無法獲取" in financials.get(
                "error"
            ):  # Check for specific fetch errors
                print(
                    f"  (ValuationState) 股票 {stock_code} ({financials.get('name', '')}) 缺少有效的財務數據，跳過估值。錯誤: {financials.get('error')}"
                )
            ...
            valuation_result = data_handler.calculate_dcf_valuation(
                stock_code, financials, risk_preference, self.context
            )
            valuation_result["name"] = financials.get(
                "name", stock_code
            )  # Add stock name for display
...
class FilteringState(State):
    def execute(self):
        ...
        print(
            f"  (FilteringState) 收到 {len(results)} 筆估值結果準備篩選。篩選潛在報酬 > {threshold*100:.1f}%"
        )
        ...
        for res in results:
            if res.get("error"):
                print(
                    f"  (FilteringState) 股票 {res.get('stock_code')} ({res.get('name', '')}) 存在估值錯誤，已跳過: {res['error']}"
                )
            ...
            if (
                ...
                or potential_return is None
            ):
                print(
                    f"  (FilteringState) 股票 {res.get('stock_code')} ({res.get('name', '')}) 缺少估值或市價數據，已跳過。"
                )
            ...
            if intrinsic_value <= 0:
                if not is_ranking_mode:
                    print(
                        f"  (FilteringState) 股票 {res.get('stock_code')} ({res.get('name', '')}) 因 內在價值 ({intrinsic_value:.2f}) <= 0 不符條件而未被選入。"
                    )
                    ...
                    print(
                        f"  (FilteringState) 股票 {res.get('stock_code')} ({res.get('name', '')}) 內在價值 ({intrinsic_value:.2f}) <= 0，但在排行模式下保留。"
                    )
            ...
            if source_eps is None or source_eps < min_eps:
                if not is_ranking_mode:
                    print(
                        f"  (FilteringState) 股票 {res.get('stock_code')} ({res.get('name', '')}) 近期會計EPS ({source_eps}) 為負或過低，已排除。"
                    )
                    ...
                    print(
                        f"  (FilteringState) 股票 {res.get('stock_code')} ({res.get('name', '')}) 近期會計EPS ({source_eps}) 為負或過低，但在排行模式下保留。"
                    )
            ...
            if enable_growth_filter:
                ...
                if not growth_result["is_growth_stock"]:
                    print(
                        f"  (FilteringState) 股票 {res.get('stock_code')} ({res.get('name', '')}) 未通過成長股條件，已篩除。"
                    )
                    ...
                    print(
                        f"  (FilteringState) 股票 {res.get('stock_code')} ({res.get('name', '')}) 通過成長股條件檢查。"
                    )
                    # 將成長股分析結果添加到結果中
            ...
            if potential_return >= threshold or is_ranking_mode:
                ...
                print(
                    f"  (FilteringState) 股票 {res.get('stock_code')} ({res.get('name', '')}) 因潛在報酬 ({potential_return:.1%}) 未達標 (> {threshold*100:.1f}%) 而未被選入 (內在價值: {intrinsic_value:.2f}, 會計EPS: {source_eps})."
                )
        ...
        if self.context.get("screening_mode") == "ranking":
            filtered_results.sort(
                key=lambda x: x.get("potential_return", -999), reverse=True
            )
        ...
        if enable_growth_filter:
            print(
                f"FilteringState 完成，成長股篩選排除了 {growth_filtered_count} 筆股票，最終篩選出 {len(filtered_results)} 筆符合條件的股票。"
            )
        else:
            print(
                f"FilteringState 完成，共篩選出 {len(filtered_results)} 筆符合條件的股票。"
            )
    ...
    def _evaluate_growth_stock(self, stock_result):
        ...
        try:
            ...
            revenue_cagr_threshold = (
                self.context.get("revenue_cagr_threshold", 15.0) / 100.0
            )  # 轉換為小數

            eps_cagr_enabled = self.context.get("eps_cagr_enabled", False)
            eps_cagr_threshold = (
                self.context.get("eps_cagr_threshold", 15.0) / 100.0
            )  # 轉換為小數

            roe_enabled = self.context.get("roe_enabled", True)
            roe_threshold = (
                self.context.get("roe_threshold", 15.0) / 100.0
            )  # 轉換為小數
            ...
            # 如果沒有啟用任何條件，視為通過
            if not criteria:
                return {"is_growth_stock": True, "details": []}
            ...
            return {
                ...
                "details": [
                    {"criterion": "評估錯誤", "value": None, "status": f"錯誤: {e}"}
                ],
            }
