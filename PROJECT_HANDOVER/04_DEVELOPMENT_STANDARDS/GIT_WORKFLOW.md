...
- [8. 最佳實踐](#8-最佳實踐)
  - [8.1 日常開發](#81-日常開發-)
  - [8.2 團隊協作](#82-團隊協作-)
  - [8.3 備份與版本控制規則](#83-備份與版本控制規則-)
  - [8.4 工具和自動化](#84-工具和自動化-)
...
**核心原則：所有項目歷史由 Git 管理，嚴禁手動備份文件**

```bash
# ✅ 正確的版本控制方式
git add .
git commit -m "feat(module): 描述變更內容"
...
1. **Git 是唯一的版本控制工具**
   - ✅ 所有代碼變更必須通過 Git commit 保存
   - ✅ 所有歷史版本都可以通過 Git 命令恢復
   - ❌ 禁止創建手動備份文件（.backup, .old, .bak 等）
   - ❌ 禁止複製整個目錄作為備份
...
2. **重要變更前的準備**
   ...
   # 危險操作前：確保已提交
   git add . && git commit -m "save: 操作前保存點"
...
3. **團隊協作中的備份原則**
   - 📝 個人開發：至少每日 commit 並 push
   - 📝 功能完成：立即合併到主分支
   - 📝 重要里程碑：創建 tag 標記
   - 📝 發布版本：通過 release 分支管理
...
5. **嚴禁的行為 ⚠️**
   - ❌ 在項目目錄內創建 `.backup` 文件夾
   - ❌ 複製 `main.py` 為 `main_backup.py`
   - ❌ 保存 `舊版本` 文件在項目內
   - ❌ 使用文件名後綴 `.old`, `.bak`, `.backup`
   - ❌ 手動保存 "工作版本" 和 "穩定版本"

#### 💡 為什麼要遵循這些規則？

- **避免混亂**：手動備份會造成版本混亂，分不清哪個是最新版本
- **節省空間**：Git 的差異儲存比完整複製更高效
- **完整歷史**：Git 記錄所有變更歷史，包括作者、時間、原因
- **團隊協作**：所有成員都能看到相同的歷史，無需傳遞備份文件
- **專業標準**：符合軟體開發行業標準實踐
