...
try:
    ...
    DCF_CALCULATOR_AVAILABLE = True
except ImportError as e:
    print(f"DCF計算器導入失敗: {e}")
    DCF_CALCULATOR_AVAILABLE = False
    ...
    with col_right:
        ...
        for i in range(5):
            rate = st.slider(
                ...
                min_value=-0.20,
                max_value=0.50,
                value=default_rates[i],                step=0.01,
                key=f"growth_{i}"
            )
...
def render_dcf_calculation(inputs):
    ...
    # 準備計算參數
    if DCF_CALCULATOR_AVAILABLE:
        try:
            # 使用真實的DCF計算器
            financial_data = {
                'net_income_parent': inputs['initial_fcf'] * 1000000,  # 轉換為元
                'shares_outstanding': inputs['shares_outstanding'] * 1000000,
                'current_market_price': 100,  # 預設值
                'cash': inputs['cash'] * 1000000,
                'debt': inputs['debt'] * 1000000
            }
            ...
            result = calculator.calculate_dcf(
                ...
                growth_rate=sum(inputs['growth_rates']) / len(inputs['growth_rates']),  # 平均成長率
            ...
            current_price = result.get('current_market_price', 100)
            
            st.metric(
                "每股內在價值",
                f"NT$ {intrinsic_value:,.2f}",
                help="基於DCF模型計算的每股價值"
            )
            ...
            if current_price > 0:
                ...
                st.metric(
                    "潛在上漲空間",
                    f"{upside:+.1f}%",
                    delta=f"vs 市價 NT$ {current_price:.2f}",
                    help="相對於目前市價的潛在報酬"
                )
...
def render_sensitivity_analysis(inputs, base_result):
    ...
    fig_heatmap = go.Figure(data=go.Heatmap(
        ...
        x=growth_labels,
        y=wacc_labels,        colorscale='RdYlGn',
        hoverongaps=False
    ))
