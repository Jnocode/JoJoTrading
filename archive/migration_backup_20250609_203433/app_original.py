...
# type: ignore
"""
JoJotrading Streamlit 主應用程式 - 備份版本

注意：這是一個備份檔案，僅供參考，不應直接運行。
當前系統使用的主要檔案為 main_app.py 和 src/jojo_trading/ui/app.py
...
try:
    from src.jojo_trading.config.dcf_optimized_config import DCF_OPTIMIZED_CONFIG  # Import optimized DCF configuration
except ImportError:
...
try:
    ...
    # 定義變數以避免類型檢查錯誤
    _get_all_taiwan_growth_presets = get_all_taiwan_growth_presets
    _get_all_taiwan_dcf_presets = get_all_taiwan_dcf_presets  
    _get_all_taiwan_industry_presets = get_all_taiwan_industry_presets
    _apply_taiwan_growth_preset = apply_taiwan_growth_preset
    _apply_taiwan_dcf_preset = apply_taiwan_dcf_preset
    _apply_taiwan_industry_preset = apply_taiwan_industry_preset
except ImportError:
    def _get_all_taiwan_growth_presets():
        return {}
    def _get_all_taiwan_dcf_presets():
        return {}
    def _get_all_taiwan_industry_presets():
        return {}
    def _apply_taiwan_growth_preset(*args, **kwargs):
        return None
    def _apply_taiwan_dcf_preset(*args, **kwargs):
        return None
    def _apply_taiwan_industry_preset(*args, **kwargs):
        return None

try:
    from src.jojo_trading.config.user_config import (
        UserConfigManager as RealUserConfigManager,
        UserGrowthConfig as RealUserGrowthConfig,
        UserDCFConfig as RealUserDCFConfig,
        UserIntegratedConfig as RealUserIntegratedConfig,
        UserConfigMetadata as RealUserConfigMetadata
    )
    # 使用實際導入的類別
    UserConfigManager = RealUserConfigManager
    UserGrowthConfig = RealUserGrowthConfig
    UserDCFConfig = RealUserDCFConfig
    UserIntegratedConfig = RealUserIntegratedConfig
    UserConfigMetadata = RealUserConfigMetadata
except ImportError:
    class UserConfigManager:
        def __init__(self):
            pass
        def save_growth_config(self, config):
            return False
        def save_dcf_config(self, config):
            return False
        def list_all_configs(self):
            return []
        def load_and_apply_config(self, config_id, context):
            return False
        def delete_config(self, config_id):
            return False
        def export_config(self, config_id, file_path):
            return False
        def import_config(self, file_path, user):
            return None
...
lang = st.sidebar.selectbox(
    t("select_language", st.session_state["language"]) or "選擇語言",
...
developer_mode_ui_switch = st.sidebar.checkbox(
    t("use_simulated_data", lang) or "使用模擬資料", 
    ...
    if preset_type and preset_type in preset_options:
        ...
        if selected_preset:
            ...
            if preset_type == "成長股篩選":
                ...
                # 可以選擇直接應用此預設                if st.sidebar.button("🚀 直接應用此產業預設"):
                    try:
                        if selected_preset_key:
                            apply_taiwan_industry_preset(selected_preset_key, machine.context)
...
# === 保存/載入配置 ===
    ...
    if saved_configs:
        ...
        if export_config_name != "選擇配置..." and st.sidebar.button("📤 匯出配置"):
            ...
            with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as temp_file:
                ...
                success = st.session_state.config_manager.export_config(export_config["id"], temp_path)
                if success:
    ...
    if uploaded_config is not None:
        try:
            ...
            # 假設當前用戶名為"匯入用戶"，實際應用中可從session或登入狀態取得
            current_user = st.session_state.get('current_user', '匯入用戶')
            config_id = st.session_state.config_manager.import_config(temp_path, current_user)
...
if machine.current_state == JoJoState.UI_INIT: # 使用 machine.current_state
    ...
    if st.button("開始篩選股票"):
        ...
        machine.context['financial_data_freq'] = selected_freq_from_ui
          # 產業模式才有潛在報酬率篩選
        if mode == "產業篩選模式":
            machine.context['potential_return_threshold'] = (min_potential_return_percentage or 0) / 100.0 # 統一用這個key
    ...
    risk_preference = machine.context.get('risk_preference', 0)
    st.info(f"接收到產業：{machine.context.get('selected_industry_name')}，風險補償：{risk_preference*100:.1f}%") # 使用 selected_industry_name
...
elif machine.current_state == JoJoState.IDLE: # 使用 IDLE 替代不存在的 END
...
def main():
    ...
    # 頁面標題
    st.header("📊 DCF 估值分析系統")
    st.markdown("**D**iscounted **C**ash **F**low 現金流折現模型股票估值工具")
    
    # 功能選擇標籤
    tab1, tab2 = st.tabs(["📈 個股DCF分析", "🎯 類股篩選DCF"])
    
    with tab1:
        render_individual_dcf_analysis()
    
    with tab2:
        render_sector_dcf_screening()


def render_individual_dcf_analysis():
    """個股DCF分析功能"""
    ...
    with col1:
        ...
        # 股票基本資訊
        stock_code = st.text_input("股票代碼", value="2330", help="請輸入台股代碼，例如：2330")
        
        # 財務數據輸入
        st.markdown("**財務數據**")
        current_price = st.number_input("目前股價 (元)", value=500.0, min_value=0.1, step=1.0)
        net_income = st.number_input("年度淨利 (億元)", value=100.0, min_value=0.1, step=1.0)
        shares_outstanding = st.number_input("流通股數 (億股)", value=25.9, min_value=0.1, step=0.1)
        
        # DCF 參數設定
        st.markdown("**DCF 估值參數**")
        discount_rate = st.slider("折現率 (%)", min_value=5.0, max_value=15.0, value=8.0, step=0.1) / 100
        growth_rate = st.slider("短期成長率 (%)", min_value=-10.0, max_value=30.0, value=8.0, step=0.1) / 100
        terminal_growth = st.slider("永續成長率 (%)", min_value=0.0, max_value=5.0, value=3.0, step=0.1) / 100
        projection_years = st.selectbox("預測年數", options=[3, 5, 7, 10], index=1)
        ...
        if st.button("🧮 執行 DCF 估值計算", type="primary"):
            try:
                ...
                financial_data = {
                    ...
                    'net_income_parent': net_income * 100000000,  # 轉換為元
                    'shares_outstanding': shares_outstanding * 100000000,  # 轉換為股
                ...
                if 'error' in result:
                    ...
                    with st.expander("📊 計算詳情"):
                        ...
                        st.write(f"**使用折現率**: {result.get('used_discount_rate', discount_rate):.1%}")
                        st.write(f"**使用成長率**: {result.get('used_growth_rate', growth_rate):.1%}")
                        st.write(f"**永續成長率**: {result.get('used_terminal_growth', terminal_growth):.1%}")
...
def render_sector_dcf_screening():
    """類股篩選DCF分析功能"""
    ...
    with col2:
        ...
        if st.button("🚀 開始類股DCF篩選", type="primary"):
            # 更新狀態機參數
            machine.context.update({
                'selected_industry_name': selected_industry,
                'risk_preference': risk_preference,
                'potential_return_threshold': potential_return_threshold,
                'dcf_short_term_growth_rate': growth_rate,
                'dcf_terminal_growth_rate': terminal_growth,
                'enable_growth_filter': enable_growth_filter
            })
            ...
            if enable_growth_filter:
                machine.context.update({
                    ...
                    'growth_logic_operator': 'AND' if 'AND' in growth_logic else 'OR'
                })
