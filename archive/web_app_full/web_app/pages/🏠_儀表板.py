...
def render_market_overview():
    ...
    # 市場情緒指標 (Real AI from News)
    sentiment_score = 50 # Default
    try:
        # We need to access shared data. 
        # Since we don't have a shared DB yet, we can instantiate the scraper/analyzer 
        # and fetch a small batch to get the 'Heat'.
        # To avoid performance hit, we cache this specific calculation.
        
        @st.cache_data(ttl=300) # Cache for 5 mins
        def get_ai_sentiment_score():
            ...
            # If Monday (0), look back to Friday (3 days ago). Else 1 day.
            now = datetime.now()
            ...
            # Use new fetch_news_since or just basic limit if not available yet?
            # We implemented fetch_news_since in scraper.
            try:
                # Fetch up to 50 items from the gap
                news = scraper.fetch_news_since(since_dt)
                # Limit to 50 for analysis performance
                news = news[:50]
            except:
            ...
            analyzed = analyzer.analyze_impact_batch(news)
            
            # Simple avg heat
            heat_scores = [item['analysis'].get('heat_score', 0) for item in analyzed if isinstance(item['analysis'].get('heat_score'), (int, float))]
            avg_heat = sum(heat_scores) / len(heat_scores) if heat_scores else 50
            
            # Optional: Weight by volume? 
            # If weekend had huge volume of bad news, average handles it.
            return int(avg_heat)

        sentiment_score = get_ai_sentiment_score()
    except Exception as e:
        # Fallback
        pass

    # Use the exposed render logic or rebuild figure?
    # Simpler to rebuild figure here to match the subplot context or use add_trace with the gauge object.
    # The NewsDashboardUI.render_gauge returns a Figure. We need a Trace.
    # So we copy the trace config here.
    ...
    fig.add_trace(
        go.Indicator(
            ...
            domain={'x': [0, 1], 'y': [0, 1]},
            title={'text': "🔥 市場熱度恐貪指數 (AI)"},
            delta={'reference': 50},
            gauge={
                'axis': {'range': [None, 100]},
                'bar': {'color': "white"},
                'sections': [ # Using 'sections' or 'steps'
