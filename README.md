...
-   **已完成**:
    ...
    *   從損益表中提取 EPS、主要營收和淨利歸母數據，並已修正財報金額單位（千元轉元）問題。
    *   實現了基於「每股淨利」作為自由現金流代理的初步 DCF 估值邏輯。
    *   初步的 Streamlit UI 框架，可驅動流程並展示估值與篩選結果。
    *   產生了專案架構的類圖和序列圖 (Mermaid 原始碼及 SVG 圖片)。
-   **待辦事項**:
    *   **尋找更詳細的財報數據源**: 目前 TWSE OpenAPI 的資產負債表數據 (`t187ap07_L_ci`) 為高度匯總數據，缺乏計算營運資本變動 (ΔWC) 所需的明細科目（如應收帳款、存貨、應付帳款）。未來需尋找能提供更詳細資產負債表及現金流量表（以計算資本支出、折舊等）的數據源。
    *   **完善 DCF 估值邏輯**: 一旦獲得更詳細的財務數據，將在 `data_handler.calculate_dcf_valuation` 中實現更標準的 FCFE 計算，納入 ΔWC、淨資本支出、淨舉債等因素。
    *   **實作 `FilteringState`**: (已初步完成) 根據估值結果篩選股票，可持續優化篩選條件。
    *   **實作 `ResultsDisplayState`**: (已初步完成) 在 Streamlit UI 中展示估值和篩選結果，可持續優化呈現方式。
