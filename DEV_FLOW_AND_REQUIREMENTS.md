...
- [x] 過濾並顯示符合條件的特價股推薦清單
- [x] **自選股估值模式**
    - [x] 允許使用者輸入單一股票代號進行 DCF 估值
    - [x] 調整 UI 以支援個股輸入與結果展示
    - [x] 擴充狀態機與資料處理邏輯以支援個股估值流程
- [x] **成長股判定機制**
    - [x] 採用「預設智能閾值」自動判定成長股（如：近3年營收CAGR > 15%、EPS CAGR > 15%、ROE > 15% 等，具體數值於UI明示，可微調）
    - [x] UI 提供多項成長指標勾選（checkbox），並可用 AND/OR 邏輯組合
    - [x] 當數據不足時，於結果中標記「數據不足」或「N/A」並簡要說明
    - [ ] 僅對成長股進行 DCF 估值
